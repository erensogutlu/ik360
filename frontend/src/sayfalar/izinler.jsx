...
const Izinler = () => {
  ...
  const getir = () => {
    fetch(`${API_URL}/izinler`, { headers: { 'yetki-token': token } })
  ...
  const kaydet = async (e) => {
    e.preventDefault();
    const url = form.id ? `${API_URL}/izinler/${form.id}` : `${API_URL}/izinler`;
  ...
  const sil = async (id) => {
    ...
    await fetch(`${API_URL}/izinler/${id}`, { method: 'DELETE', headers: { 'yetki-token': token } });
    getir();
  };
