...
const Bordro = () => {
  ...
  const getir = () => {
    fetch(`${API_URL}/bordro`, { headers: { 'yetki-token': token } })
  ...
  const kaydet = async (e) => {
    e.preventDefault();
    const url = form.id ? `${API_URL}/bordro/${form.id}` : `${API_URL}/bordro`;
  ...
  const sil = async (id) => {
    ...
    await fetch(`${API_URL}/bordro/${id}`, { method: 'DELETE', headers: { 'yetki-token': token } });
    getir();
  };
